...
const publish = () => {
    return db.any(`
        ...
        WHERE 
            (departure >= (CURRENT_TIMESTAMP - INTERVAL '0:00:15') AND departure <= (CURRENT_TIMESTAMP + INTERVAL '0:05:00'))
            OR (arrival >= (CURRENT_TIMESTAMP - INTERVAL '0:00:15') AND arrival <= (CURRENT_TIMESTAMP + INTERVAL '0:05:00'))
    `).then((rows) => {
        const message = [];

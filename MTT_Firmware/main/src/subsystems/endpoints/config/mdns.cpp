...
esp_err_t WebServer::configGetMDNSHost(httpd_req_t* req) {
    ...
    ESP_RETURN_ON_ERROR(
        ...
        kTag, "GET /config/mdns/host: cannot respond"
    );
    return ESP_OK;
}
...
esp_err_t WebServer::configSetMDNSHost(httpd_req_t* req) {
    ...
    ESP_RETURN_ON_ERROR(
        ...
        kTag, "POST /config/mdns/host: error occurred reading request body"
    );
    value[len] = '\0';

    esp_err_t err = Config::setMDNSHostname(value);
    if (err != ESP_OK) {
        ESP_LOGE(kTag, "POST /config/mdns/host: error occurred writing to NVS");
        ESP_RETURN_ON_ERROR(
            httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, ""),
            kTag, "POST /config/mdns/host: cannot send error"
        );
        return ESP_FAIL;
    }
    ...
    ESP_RETURN_ON_ERROR(
        ...
        kTag, "POST /config/mdns/host: cannot send response"
    );
...
esp_err_t WebServer::configGetMDNSInst(httpd_req_t* req) {
    ...
    ESP_RETURN_ON_ERROR(
        httpd_resp_send(req, result, strlen(result)),
        kTag, "GET /config/mdns/inst: cannot respond"
    );
    return ESP_OK;
}
...
esp_err_t WebServer::configSetMDNSInst(httpd_req_t* req) {
    ...
    ESP_RETURN_ON_ERROR(
        readRequestBody(req, (uint8_t*) value, sizeof(value), &len),
        kTag, "POST /config/mdns/inst: error occurred reading request body"
    );
    value[len] = '\0';

    esp_err_t err = Config::setMDNSInstanceName(value);
    if (err != ESP_OK) {
        ESP_LOGE(kTag, "POST /config/mdns/inst: error occurred writing to NVS");
        ESP_RETURN_ON_ERROR(
            httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, ""),
            kTag, "POST /config/mdns/inst: cannot send error"
        );
        return ESP_FAIL;
    }
    ...
    ESP_RETURN_ON_ERROR(
        httpd_resp_send(req, NULL, 0),
        kTag, "POST /config/mdns/inst: cannot send response"
    );
